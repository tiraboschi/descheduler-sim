...
class ScenarioExecutor:
    ...
    def __init__(self, scenario_name: str, scenario_spec: Dict[str, Any], namespace: str = "default"):
        ...
        # Per-VM locks prevent concurrent _update_vm_utilization calls from
        # racing on the K8s patch (read-modify-write is not atomic).
        self._vm_locks: Dict[str, threading.Lock] = {}
        self._vm_locks_mutex = threading.Lock()
    ...
    def _is_in_active_window(self, gen_config: Dict[str, Any]) -> bool:
        ...
        for window in windows:
            ...
            if days is not None:
                # For overnight windows (start > end) the after-midnight portion
                # (00:00 – end) belongs to the window that opened the previous day,
                # so compare against (current_dow - 1) % 7 in that half.
                if start > end and current_hm <= end:
                    check_dow = (current_dow - 1) % 7
                else:
                    check_dow = current_dow
                allowed = [_DAY[d] for d in days if d in _DAY]
                if check_dow not in allowed:
                    continue
    ...
    def _get_vm_lock(self, vm_id: str) -> threading.Lock:
        with self._vm_locks_mutex:
            if vm_id not in self._vm_locks:
                self._vm_locks[vm_id] = threading.Lock()
            return self._vm_locks[vm_id]

    def _update_vm_utilization(self, vm_id: str):
        """Update VM utilization based on active tasks."""
        with self._get_vm_lock(vm_id):
            self._update_vm_utilization_locked(vm_id)

    def _update_vm_utilization_locked(self, vm_id: str):
...
class ScenarioController:
    ...
    def _handle_modified(self, name: str, scenario: Dict[str, Any]):
        """Restart the executor when the spec actually changes.

        Status-update MODIFIED events (generated by the executor's own
        update_status calls) are ignored so we don't restart in a loop.
        """
        new_spec = scenario.get('spec', {})

        if name not in self.executors:
            self._handle_added(name, scenario)
            return

        if new_spec == self.executors[name].spec:
            logger.debug(f"Scenario {name} modified (status update only — no restart)")
            return

        logger.info(f"Scenario {name} spec changed — stopping executor and restarting")
        self.executors[name].running = False
